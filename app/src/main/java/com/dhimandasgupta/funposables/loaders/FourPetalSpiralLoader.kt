...
@Preview
@Composable
fun FourPetalSpiralLoader(modifier: Modifier = Modifier) {
  ...
  val pulse by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 4200, easing = LinearEasing), RepeatMode.Restart),
      label = "pulse",
    )
  val rotation by
    infiniteTransition.animateFloat(
  ...
  Canvas(modifier = modifier.size(200.dp).graphicsLayer { rotationZ = rotation }) {
    ...
    drawPath(
      ...
      style = Stroke(4.4f.dp.toPx(), cap = StrokeCap.Round),
    )
