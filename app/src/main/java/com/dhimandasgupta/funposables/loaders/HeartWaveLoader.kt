...
fun HeartWaveLoader(modifier: Modifier = Modifier) {
  ...
  val progress by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 8400, easing = LinearEasing), RepeatMode.Restart),
      label = "progress",
    )
  val pulse by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 5600, easing = LinearEasing), RepeatMode.Restart),
      label = "pulse",
    )
  ...
  Canvas(modifier = modifier.size(200.dp)) {
    ...
    drawPath(
      ...
      style = Stroke(3.9f.dp.toPx(), cap = StrokeCap.Round),
    )
