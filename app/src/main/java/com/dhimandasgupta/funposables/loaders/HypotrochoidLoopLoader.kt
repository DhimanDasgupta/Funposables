...
fun HypotrochoidLoopLoader(modifier: Modifier = Modifier) {
  ...
  val progress by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 7600, easing = LinearEasing), RepeatMode.Restart),
      label = "progress",
    )
  val pulse by
    infiniteTransition.animateFloat(
  ...
  Canvas(modifier = modifier.size(200.dp)) {
    ...
    for (i in 0..480) {
      ...
      if (i == 0) path.moveTo(cx * canvasScale, cy * canvasScale)
      else path.lineTo(cx * canvasScale, cy * canvasScale)
    }
    drawPath(
      path,
