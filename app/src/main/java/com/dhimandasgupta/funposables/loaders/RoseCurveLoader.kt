...
@Preview
@Composable
fun RoseCurveLoader(modifier: Modifier = Modifier) {
  ...
  val progress by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 5400, easing = LinearEasing), RepeatMode.Restart),
      label = "progress",
    )
  val pulse by
    infiniteTransition.animateFloat(
  ...
  Canvas(modifier = modifier.size(200.dp).graphicsLayer { rotationZ = rotation }) {
    ...
    drawPath(
      ...
      style = Stroke(4.5f.dp.toPx(), cap = StrokeCap.Round),
    )
