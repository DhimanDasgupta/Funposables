...
@Preview
@Composable
fun RoseOrbitLoader(modifier: Modifier = Modifier) {
  ...
  val pulse by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 4600, easing = LinearEasing), RepeatMode.Restart),
      label = "pulse",
    )
  val rotation by
    infiniteTransition.animateFloat(
  ...
  Canvas(modifier = modifier.size(200.dp).graphicsLayer { rotationZ = rotation }) {
    ...
    for (i in 0..480) {
      val t = (i / 480f) * TWO_PI
      val r = orbitRadius - detailAmplitude * detailScale * cos(k * t)
      val x = (50f + cos(t) * r * curveScale) * cs
      val y = (50f + sin(t) * r * curveScale) * cs
      if (i == 0) path.moveTo(x, y) else path.lineTo(x, y)
    }
    drawPath(
      path,
      Color.White.copy(alpha = 0.1f),
      style = Stroke(5.2f.dp.toPx(), cap = StrokeCap.Round),
    )
    ...
    for (i in 0 until particleCount) {
      ...
      val t = p * TWO_PI
      val r = orbitRadius - detailAmplitude * detailScale * cos(k * t)
      val px = (50f + cos(t) * r * curveScale) * cs
      val py = (50f + sin(t) * r * curveScale) * cs
