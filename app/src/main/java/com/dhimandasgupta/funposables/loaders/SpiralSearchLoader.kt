...
fun SpiralSearchLoader(modifier: Modifier = Modifier) {
  ...
  val progress by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 7800, easing = LinearEasing), RepeatMode.Restart),
      label = "progress",
    )
  val pulse by
    infiniteTransition.animateFloat(
      ...
      targetValue = 1f,
      animationSpec =
        infiniteRepeatable(tween(durationMillis = 6800, easing = LinearEasing), RepeatMode.Restart),
      label = "pulse",
    )
  ...
  Canvas(modifier = modifier.size(200.dp)) {
    ...
    fun spiralPoint(prog: Float): Offset {
      ...
      return Offset(
        (50f + cos(angle) * radius * sScale) * cs,
        (50f + sin(angle) * radius * sScale) * cs,
      )
    }
    ...
    drawPath(
      ...
      style = Stroke(4.3f.dp.toPx(), cap = StrokeCap.Round),
    )
