...
@Preview
@Composable
fun ThinkingNineLoader(modifier: Modifier = Modifier) {
  ...
  val progress by
    infiniteTransition.animateFloat(
      ...
      animationSpec =
        infiniteRepeatable(
          animation = tween(durationMillis = 4700, easing = LinearEasing),
  ...
  Canvas(modifier = modifier.size(200.dp).graphicsLayer { rotationZ = rotation }) {
    ...
    for (i in 0..480) {
      ...
      val x =
        center.x +
          (baseRadius * cos(t) - detailAmplitude * detailScale * cos(petalCount * t)) *
            curveScale *
            (size.width / 200f)
      val y =
        center.y +
          (baseRadius * sin(t) - detailAmplitude * detailScale * sin(petalCount * t)) *
            curveScale *
            (size.height / 200f)
