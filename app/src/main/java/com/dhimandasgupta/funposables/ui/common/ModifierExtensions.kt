 ...
 *
 * This modifier uses [drawWithCache] to efficiently cache the grid path and re-draws it over the
 * existing content. The grid consists of vertical and horizontal lines spaced according to the
 * provided [gridSize].
 *
...
/**
 * Adds a vertical scrollbar to a LazyColumn to improve usability for scrolling. The scrollbar is
 * visually represented and customized through parameters like width, color, and thumb size.
 *
 * @param lazyListState The state object that controls and observes the scroll position of the
 *   LazyColumn.
 * @param width The total width of the scrollbar including the track. Defaults to 4.dp.
 * @param thumbWidth The width of the scrollbar's thumb, the draggable part. Defaults to the same
 *   value as `width`.
 * @param color The color of the scrollbar thumb. Defaults to a semi-transparent black.
 * @param minThumbHeight The minimum height of the scrollbar thumb, ensuring it remains visible for
 *   large content. Defaults to 32.dp.
 * @param endPadding Padding between the end of the scrollbar and the edge of the LazyColumn
 *   content. Defaults to 2.dp.
...
fun Modifier.verticalScrollbarForLazyColumn(
  ...
  return this then
    LazyListVerticalScrollbarElement(
      lazyListState = lazyListState,
      width = width,
      thumbWidth = thumbWidth,
      color = color,
    ...
    scrollJob = coroutineScope.launch {
      snapshotFlow {
          ...
          if (visibleItems.isEmpty() || totalItemsCount == 0) {
            ...
            val maxScrollOffset = (estimatedContentSize - viewportSize).coerceAtLeast(1f)

            val scrollProgress = (estimatedScrollOffset / maxScrollOffset).coerceIn(0f, 1f)

            val viewportToContentRatio = (viewportSize / estimatedContentSize).coerceIn(0f, 1f)
...
private fun ContentDrawScope.drawScrollbar(
  ...
  val thumbHeight =
    (viewportHeight * viewportToContentRatio)
...
/**
 * Applies a bottom fade gradient to a column, based on the given scroll state and fading
 * parameters. The gradient will appear only when the column is not fully scrolled to the bottom.
 *
 * @param scrollState The scroll state of the column to determine when to apply the fade effect.
 * @param heightPercentage The percentage of the column's height over which the fade effect will be
 *   applied. The default is 0.15f.
...
/**
 * Applies a fading effect at the bottom of a LazyColumn based on its scroll state. The fade effect
 * is controlled by customizable properties such as height percentage, color, and maximum opacity,
 * and is displayed when the LazyColumn can scroll forward.
 *
 * @param lazyListState The state object associated with the LazyColumn, which tracks its scroll
 *   position.
 * @param heightPercentage The height of the fade effect as a percentage of the LazyColumn's total
 *   height. Must be between 0.0f and 1.0f. Defaults to 0.15f.
 * @param color The color of the fade effect. Defaults to Color.Unspecified.
 * @param maxAlpha The maximum alpha (opacity) of the fade effect. Must be between 0.0f and 1.0f.
 *   Defaults to 0.8f.
