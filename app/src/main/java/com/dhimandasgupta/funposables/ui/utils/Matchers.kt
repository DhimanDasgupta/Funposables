      ...
      is TextLinkMatch.Custom -> {
        withLink(
          LinkAnnotation.Clickable(
            tag = match.text,
            styles =
              if (params.customLinkShouldBeUnderlined) linkStyles(params.customLinkColor)
              else noStyle(),
  ...
  val customMatches =
    if (clickListeners.isNotEmpty()) {
      ...
      if (patterns.isNotEmpty()) {
        val combinedPattern =
          patterns.sortedByDescending { it.length }.joinToString("|") { Regex.escape(it) }
...
private fun Char.toPhoneKeypadDigit() =
  when (uppercaseChar()) {
    'A',
    'B',
    'C' -> '2'
...
fun noStyle() = TextLinkStyles(style = SpanStyle())

fun linkStyles(linkColor: Color) =
  TextLinkStyles(
    style =
      SpanStyle(
        color = linkColor,
...
private val VanityPhoneRegex =
  Regex(
    pattern =
      "(?<!\\w)(?:\\+?1$PHONE_SEP?)?\\(?\\d{3}\\)?$PHONE_SEP?" +
